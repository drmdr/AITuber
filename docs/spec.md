...
- **コミットとプッシュ**: 機能実装、バグ修正、ドキュメント更新などのすべての変更作業が完了した後、Gitへのコミットと`master`ブランチへのプッシュは、原則としてAIアシスタント（Cascade）が実行します。ユーザー様が手動でプッシュする必要はありません。
...
- **アバター表示・動作の仕組み**:
  - このAITuberシステムは、キャラクターの3Dモデルを直接レンダリングする機能は持ちません。
  - アバターの表示とリップシンクは、外部アプリケーションの連携によって実現されます。
    1. **VRoid Studio**: キャラクター「モナミン」の3Dモデル（`.vrm`形式）を作成します。
    2. **VSeeFace**: VRoidで作成した`.vrm`モデルを読み込み、リップシンクを有効にします。
    3. **AITuberシステム**: `AITuber.py`がGemini API等で生成したセリフを音声合成し、PCの音声出力デバイスから再生します。
    4. **OBS Studio**: VSeeFaceのウィンドウをキャプチャし、背景などと合成して最終的な映像として配信・録画します。
  - **重要**: このため、本プロジェクトのリポジトリ内には、キャラクターの画像ファイル（PNG, JPG等）は原則として含まれません。X Posterで画像を生成する際は、参照画像を用いず、詳細なテキストプロンプトでキャラクターの特徴をAIに指示する必要があります。
- **主要機能**:
  - テキストベースのチャット応答
  - 音声合成 (Gemini API TTS / Google AI Studio TTS)
- **使用する可能性のある外部サービス**:
  - OpenAI API (チャ- **Google Cloud Project ID**: `*******` (monamin-aituber)
...
### 2025-07-03: X Poster 画像生成機能のデバッグ

- **課題**: X Poster (`morning_greet_poster.py`) に実装されたVertex AI Imagenによる画像生成機能で、キャラクター「モナミン」の見た目が安定しない。
- **初期アプローチ（誤）**: `backend/avatar/` 内に存在する画像をキャラクターの参照画像として使用し、`edit_image` APIで画像を生成しようと試みた。しかし、これは以下の点で根本的に誤っていた。
  1. ユーザーより「`backend`フォルダは現在使用していない」との指摘。
  2. ユーザーより「AITuberのアバターはVSeeFaceとVRoidモデルで動作させており、リポジトリ内に画像ファイルは存在しない」との指摘。
- **修正アプローチ（正）**: 参照画像を使用するロジックを完全に撤廃。代わりに、`generate_images` APIを使用し、詳細なテキストプロンプトによってキャラクターの特徴をAIに直接指示する方式に修正した。
- **残タスク**: ユーザーから「モナミン」の詳細な外見的特徴（髪、目、服装など）をヒアリングし、`generate_and_save_image`関数内のプロンプトを更新する必要がある。
